...
def get_pixel_matching_threshold(obsra, modelra, threshold=0):
    ...
    if numobs < len(modelx):
        ...
        print(
            "Warning Pixel Matching: More observed than modeled values above threshold"
        )
    if np.isnan(modelx[0]):
        print("WARNING Pixel Matching: threshold is nan")
    return modelx[0]
...
def get_ds(n1, n2):
    ...
    ds = 0.5 * (1.0 / n1 + 1.0 / n2) * np.log(alpha / 2)
    ds = (-1 * ds) ** 0.5
    return alpha, ds
...
def twosample_kstest(data1, data2):
...
def pixel_matched_cdf(modelra, numobs, threshold=0):
    ...
    modely = 1.0 * np.arange(modelx.size) / float(modelx.size - 1)
...
def cdf(data):
    ...
    yval = 1.0 * np.arange(sdata.size) / float(sdata.size - 1)
    return sdata, yval
...
class EnsembleTimeSeries:
    def __init__(
        self, obs=pd.Series(dtype="float64"), fc=pd.Series(dtype="float64"), stn=None
    ):
...
class MatchedData(object):
    ...
    @property
    def fc(self):
        return self.obsra.fc
 
    @property
    def obs(self):
        return self.obsra.obs
    ...
    def create_obsra(self, obs, fc):
        ...
        data.columns = ["obs", "fc"]
        # if no forecast but obs then make forecast 0.
        # if forecast but no obs then 
        fc = data.fc
        obs = data.obs
        fc = fc.fillna(0)
        data = pd.concat([obs,fc],axis=1) 
        ...
        data = data.dropna(axis=0)  # replace nan with 0
        return data
    ...
    def plotseries(self, ax, clrs, obs=True, lbl="forecast", mult=1,lw=2,alpha=1):
        """
        plots the obsra
        """
        time = self.obsra.index.tolist()
        if obs:
            ax.plot(time, self.obsra["obs"], clrs[0], label="observations", linewidth=lw)
        ax.plot(time, self.obsra["fc"] * mult, clrs[1], label=lbl, alpha=alpha,linewidth=4)
    ...
    def computediff_degrees(self):
        ...
        temp["diff"] = temp.apply(lambda row: diff(row["obs"], row["fc"]), axis=1)
...
def find_stats_function(mra, scale=1, thresh=0, bg=0):
    ...
    for mm, cc in zip(obs, fc):
        ...
        suma += np.abs(mm - cc)  #measured - observed
        sumf += mm - cc  #measured - observed
        # mse += (pair[0] - pair[1])**2
        # nnn+=1
        # obsave += pair[0]
        # fcave += pair[1]
