...
def setup_figure(fignum=1,rows=1,columns=1,central_longitude=180,figsize=[10,5]):
    transform = get_transform(central_longitude=central_longitude)
    fig,axra = plt.subplots(figsize=figsize,nrows=rows,ncols=columns,
                                constrained_layout=False, subplot_kw={'projection':transform})
    return fig, axra
...
# our data always has central longitude = 0
def data_transform():
